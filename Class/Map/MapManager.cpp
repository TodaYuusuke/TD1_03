	...
	if (isBoss) {
		...
		if (isBlock) {
			if (tutorialMap[y][x]->CheckHitBox(hitPosition, isBlock)) {
				// 周囲のブロックをまとめて破壊
				if (y - 1 >= 0) {
					if (tutorialMap[y - 1][x]->type == kTypeWeak) {
						tutorialMap[y - 1][x]->type = kTypeAir;
						PublicFlag::kBrokenConcretePlaySE = true;
					}
					if (x - 1 >= 0) {
						if (tutorialMap[y - 1][x - 1]->type == kTypeWeak) {
							tutorialMap[y - 1][x - 1]->type = kTypeAir;
							PublicFlag::kBrokenConcretePlaySE = true;
						}
					}
				...
				if (x - 1 >= 0) {
					if (tutorialMap[y][x - 1]->type == kTypeWeak && x - 1 >= 0) {
						tutorialMap[y][x - 1]->type = kTypeAir;
						PublicFlag::kBrokenConcretePlaySE = true;
					}
				}
				if (x + 1 < BaseConst::kTutorialStageSizeWidth) {
					if (tutorialMap[y][x + 1]->type == kTypeWeak && x + 1 < BaseConst::kTutorialStageSizeWidth) {
						tutorialMap[y][x + 1]->type = kTypeAir;
						PublicFlag::kBrokenConcretePlaySE = true;
					}
				}
